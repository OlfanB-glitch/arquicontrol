...
class DashboardService:
    ...
    async def get_summary(self) -> dict:
        ...
        projects = await proyecto_service.list_all()
        payments = await proyecto_service.get_payments_feed()
        purchases = await proyecto_service.get_purchases_feed()
        ...
        return {
            ...
            "saldoPendientePorProyecto": sorted(balances, key=lambda item: item["saldoPendiente"], reverse=True),
            ...
            "eventosRecientes": sorted(eventos_recientes, key=lambda item: item.get("fecha", ""), reverse=True)[:10],
        }
